...
- [x] ✅ **B5 Deploy** (DONE 2026-04-24, commit cc08530) — Railway backend live at `web-production-b78bb.up.railway.app`, Nixpacks builder, OAuth origin configured, env vars set
...
| 🟦 FORGE | focused square 2×2 | set jaw (full row) |
...
> **Summary only** — full details ที่ `~/.claude/agents/neo.md`
...
Entries ที่ค้าง context สำคัญ — ทำไมเลือก A ไม่ B

### 2026-04-24: web/ restructure
- **เลือก:** ย้าย 10 HTML + 7 JS → `web/` folder, HTML pages ไม่ nest ใน pages/
- **ไม่เลือก:** nest เป็น web/pages/ + web/js/
- **เหตุผล:** keep flat — cross-page links ยังเป็น `neo-labs-xxx.html` ไม่ต้องแก้ relative paths, FastAPI mount `/` ชี้ `web/` โดยตรง

### 2026-04-24: root requirements.txt + runtime.txt (restored)
- **เก็บ:** ไฟล์ 2 ตัวนี้ที่ root แม้มี api/requirements.txt
- **Why:** Railway Nixpacks detect Python ด้วย root-level marker — ลบแล้ว build fail
- **Lesson:** ห้ามลบ deploy config โดยไม่ test deploy ก่อน

### 2026-04-19: FastAPI at port 8000 serve static + API
- **เลือก:** single port (FastAPI `app.mount("/", StaticFiles)`)
- **ไม่เลือก:** separate nginx/vercel static + backend
- **เหตุผล:** Google OAuth origin single — `http://localhost:8000` ง่าย, Railway deploy 1 service ถูก

### 2026-04-17: Option C (Hybrid) ก่อน Option B (Full API)
- **เลือก:** ทำ C ก่อน (Claude Code + hook + log file)
- **Why:** ใช้ได้ทันที ฟรี, ยังไม่ต้อง $20 API credit

## Error Patterns (Known gotchas)

ดูรายละเอียดเต็มที่ memory file — นี่คือ summary

1. **Anthropic API traps** → `feedback_anthropic_gotchas.md`
   - model alias ≠ API ID (`claude-opus-4-7` bare → API `claude-opus-4-7[1m]`)
   - `BaseExceptionGroup` import path แตก per version
   - Role alternation strict (ห้าม user→user)
   - Env var sanitize — strip `=` ที่ paste พลาด

2. **Railway Nixpacks** → `project_neo_labs_deploy.md`
   - root `requirements.txt` + `runtime.txt` **ห้ามลบ** — language detection

3. **Investigate before fix** → `feedback_investigate_before_fix.md`
   - รายงานคลุมเครือ = delegate agent ตรวจสาเหตุก่อนแก้

## Code Style Preferences (บอส)

- **ภาษา:** Thai-first (UI + docs), English สำหรับ technical term
- **Comment ใน code:** ทุกไฟล์มี comment สั้นอธิบาย what + why
- **Identifier:** camelCase (JS), snake_case (Python), ภาษาอังกฤษ
- **CSS:** short class names (.tb, .rc, .mitem) + CSS variables (:root)
- **Format:** ไม่มี trailing spaces, newline at EOF
- **Async:** Python → `async def` + `await`, JS → `async/await` ไม่ใช่ `.then()`
- **Error handling:** only at boundaries (user input, external APIs) — trust internal code
